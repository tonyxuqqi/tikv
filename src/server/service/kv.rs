...
impl<E: Engine, L: LockManager, F: KvFormat> Tikv for Service<E, L, F> {
    ...
    fn raft(
        ...
        let extension = self.storage.get_engine().raft_extension().clone();
        ...
        let res = async move {
            ...
            while let Some(msg) = stream.try_next().await? {
                ...
                if let Err(err @ RaftStoreError::StoreNotMatch { .. }) =
                    Self::handle_raft_message(store_id, &extension, msg, reject)
                {
    ...
    fn split_region(
        ...
        split_keys.sort();
        let f = self.storage.get_engine().raft_extension().split(
            region_id,
...
fn future_flashback_to_version<E: Engine, L: LockManager, F: KvFormat>(
    ...
    async move {
        ...
        if let Some(err) = extract_region_error(&v) {
            ...
            let res = storage_clone.get_engine().end_flashback(req.get_context());
            res.await?;
        }
        Ok(resp)
    }
