...
pub struct MultiRocksEnginesFactory<RocksEngine, R> {
    kv: RocksEngine,
    raft: R,
    tablet_factory: Box<dyn TabletFactory<RocksEngine> + Send>,
}

impl<R: RaftEngine> EnginesFactory<RocksEngine, R> for MultiRocksEnginesFactory<RocksEngine, R> {
    fn create_engines(&self, region_id: u64, suffix: u64) -> Result<Engines<RocksEngine, R>> {
...
impl<R: RaftEngine> Clone for MultiRocksEnginesFactory<RocksEngine, R> {
    fn clone(&self) -> Self {
