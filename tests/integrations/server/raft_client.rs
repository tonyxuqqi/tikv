...
fn test_notify_early() {
    ...
    let queue_capacity = raft_client.queue_capacity();
    ...
    raft_client.flush();
    check_msg_count(500, &msg_count, queue_capacity * 3 + 1);
    drop(raft_client);
    drop(mock_server);
    assert_eq!(msg_count.load(Ordering::SeqCst), queue_capacity * 3 + 1);
}
