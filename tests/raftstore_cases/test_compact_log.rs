...
fn test_compact_log<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    for (&id, engines) in &cluster.engines {
        let mut state: RaftApplyState =
            get_msg_cf_or_default(&engines.kv_db, CF_RAFT, &keys::apply_state_key(1));
...
fn check_compacted(
    ...
    for (id, engines) in all_engines {
        for i in 0..compacted_idx[id] {
            ...
            if engines.raft_db.get(&key).unwrap().is_none() {
                break;
            }
            assert!(engines.raft_db.get(&key).unwrap().is_none());
        }
    }
    true
}
...
fn test_compact_count_limit<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    for (&id, engines) in &cluster.engines {
        must_get_equal(&engines.kv_db, b"k1", b"v1");
        let mut state: RaftApplyState =
            get_msg_cf_or_default(&engines.kv_db, CF_RAFT, &keys::apply_state_key(1));
