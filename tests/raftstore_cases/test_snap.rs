...
fn test_concurrent_snap<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    must_get_equal(&cluster.get_kv_db(3), b"k3", b"v3");
    // Ensure the regions work after split.
    cluster.must_put(b"k11", b"v11");
    must_get_equal(&cluster.get_kv_db(3), b"k11", b"v11");
    cluster.must_put(b"k4", b"v4");
    must_get_equal(&cluster.get_kv_db(3), b"k4", b"v4");
}
...
fn test_cf_snapshot<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    let engine1 = cluster.get_kv_db(1);
...
fn test_node_stale_snap() {
    ...
    pd_client.must_add_peer(r1, new_peer(3, 3));
    cluster.must_put(b"k2", b"v2");
    must_get_equal(&cluster.get_kv_db(3), b"k2", b"v2");
    pd_client.must_remove_peer(r1, new_peer(3, 3));
    must_get_none(&cluster.get_kv_db(3), b"k2");
    pd_client.must_add_peer(r1, new_peer(3, 4));

    cluster.must_put(b"k3", b"v3");
    must_get_equal(&cluster.get_kv_db(2), b"k3", b"v3");
    rx.recv().unwrap();
    sleep_ms(2000);
    must_get_none(&cluster.get_kv_db(3), b"k3");
    cluster.clear_send_filters();
    must_get_equal(&cluster.get_kv_db(3), b"k3", b"v3");
}
...
fn test_snapshot_with_append<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.must_put(b"k1", b"v1");
    cluster.must_put(b"k2", b"v2");
    let engine4 = cluster.get_kv_db(4);
